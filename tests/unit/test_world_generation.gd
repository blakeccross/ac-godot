...
func test_authored_test_town_has_fixed_layout() -> void:
	...
	assert_that(_building_visual(data, &"npc_house_0")).is_equal(&"obj_s_house1_b")
...
func test_generated_town_has_ac_structure() -> void:
	...
	## Procedural NPC homes get per-resident shape/palette after starter assign.
	var npc0: BuildingPlacement = _building(data, &"npc_house_0")
	assert_that(npc0.resident_id).is_not_equal(&"")
	var npc0_data: VillagerData = VillagerCatalog.get_villager(npc0.resident_id)
	assert_that(npc0.visual_id).is_equal(npc0_data.outdoor_house_visual())
	...
	for i: int in WorldGenerator.STARTER_NPC_HOUSES:
		...
		var house_villager: VillagerData = VillagerCatalog.get_villager(npc_b.resident_id)
		assert_that(npc_b.visual_id).is_equal(house_villager.outdoor_house_visual())
