...
| `src/data/scene/police_box.c` | Scene player_data (not outdoor enter) |
| `src/actor/ac_police_box.c` | Outdoor enter `{200,0,380}` `mSc_DIRECT_NORTH` |
...
- Letter write uses preset bodies + a recipient picker rather than a full stationery editor. Bank amount entry is choice presets (All / 1,000), not the money keypad overlay. House **loan** lives on `Inventory.loan`; when `loan > 0`, Pelly's Deposit choice is repay, and bank savings unlock once loan is 0. PTerminal plays the exact eTM welcome / choices; GBA link answers with the "not properly connected" bank lines (`msg_15859`). Booker stands at his actable spot rather than wandering a zone, and claim confirm is immediate.
