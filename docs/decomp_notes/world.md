...
**Godot:** Beach/marine and open-ocean acres keep **imported GLB materials** for land. Wet-sand OPA (`beachA`) stays in marine GLBs and uses `beach_wet.gdshader`: GX `(PRIM−ENV)×I+ENV` from I4 alpha in 8-bit space as **lit** `ALBEDO` (same kankyo sun/ambient/fog as dry sand / FG props — original is BG OPA with `G_LIGHTING` + `G_FOG`). Do not add an env delta onto the baked albedo, and do not draw it unshaded. Ocean-floor underdraw (`beachB`) uses the same shader with the dark-blue prim. XLU ocean waves are kept on every acre that draws them — nearshore marine acres get both bands (shore `wave2` CLAMP T + open `wave3` REPEAT), open-ocean border acres get the open band — and all of them run `ocean_water.gdshader` over the `beachB` underdraw. That shader samples **linear**, not nearest: `m_rcp` sets `G_TF_BILERP` globally, and point-sampling 32px cell maps stretched over an acre turns the soft crests into single-pixel cracks. River and splash surfaces still use `river_water` / `splash_water`. Inland river acres use `river_water.gdshader`. Outdoor fog stays in `world.gd`.
