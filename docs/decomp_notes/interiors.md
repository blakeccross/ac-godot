...
- Nook and Able Sisters interiors spawn a counter plus that day's goods (mannequins at Able). Those nodes are not `Room.placements`, so they are not decorate-able. Post office and police box are authored the same way (`post_office.tscn` / `police_box.tscn`) with Pelly/Phyllis or Booker plus mail piles / lost-and-found (`docs/decomp_notes/post_police.md`). Public room shells are authored under `scenes/world/interiors/` (and museum wings under `scenes/world/museum/`); `interior.tscn` mounts them via `InteriorCatalog.scene_path`.
