...
| Dock sign | FG **`PORT_SIGN`** (`0x5852`) on `grd_s_m_wf_*` at unit **(8, 7)** on `_1`/`_2`, **(9, 7)** on `_3`. Visual is a flat **`dock_sign.glb`** billboard: `obj_s_kanban_base_tex` wood frame composited with a bulletin note (white paper, red tack, scribbles). Not `obj_shop_kanban` and not the player `SIGNBOARD` two-quad mesh. |
