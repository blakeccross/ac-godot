...
Two branches of that message flow are modelled differently. `mSM_CHECK_LAST_FISH_GET` swaps in `0x1349` once the species is already in `MuseumBook`; we do that at hook time. And when pockets are full `mMsg_Set_continue_msg_num(win, 0x1348)` chains "But my pockets are full! / Should I swap it for something else?" onto the report with a two-option choice; we show that line after the species report and drop the choice. `Fishing` still puts the fish in your pockets at hook time, though — the original does it in `notice_rod`'s setup, which is why a full pocket there gets a swap-or-discard choice we do not offer. An empty or escaped line keeps its plain `Game.post_notice`: there is no pose to hang it on, and the original shows no message at all for reeling in nothing.
