...
The field type is a **step machine**, not a single wander loop: leave house (hidden) → wander. If they are already outside (`is_home == FALSE`), skip leave and wander immediately. Wander think **does not end** until the schedule type changes (or a pitfall interrupt). Each wander step rolls wait / walk / run from looks weights and walks to a dest on a circle around the **acre center** (`range_radius` 280 GX). That dest is the world point on the circle, not a unit center. Arrival compares dist² to **72 GX** (~0.42 m); a dest more than 90° behind is a turn in place. Hitting a wall is `aNPC_avoid_wall` (2 units at 22.5° / 45° / 90°, then 180°) — they do not grind a collider. In-house is go home → into house → hide. Sleep hides when `is_home`. While in the field, `m_npc_walk` picks a goal acre from looks-based `{shrine, home, alone, my_home}` tables. Assigned walkers (cap `n/3`) walk toward that acre; everyone else keeps wandering their current acre.
...
- Field goals use `mNpcW_GOAL_*` kinds and acre picks. Full-town walks the route; no acre-edge appear/streaming and no gate waypoint graphs. Stay-in-acre then new goal is ~28s, not the original 30-minute arrive counter. Wander dests sit on the 280 GX acre circle (not snapped to a cell). Arrive uses √72 GX so a walk covers the rim. A wall hit steers 2 units off (`aNPC_avoid_wall`). Display names use the original villager names; meshes are disc species GLBs (`squ_1`, `cat_1`, …) when converted.
