...
- **Head look:** while outdoors (and indoors when home), head tracks the player if xz &lt; 120 GX (6 m) and within ±67.5° of body facing (`NpcHeadLook` / `aNPC_check_look_player`). Target is the player's **eye** (`Actor_world_to_eye(player, 33)` = 1.65 m), not their feet; yaw ±67.5°, pitch clamped only looking up (−33.75°). Talk requests the head at priority 4 and tracks past the range/cone gates (`head.lock_flag` is never set by talk). Sleepy mood freezes the head angles (`aNPC_check_condition_search_eye`), no return to neutral. With no target the head chases back to the **animation's own** head rotation (`joint_angle`), not zero; the override replaces only rot.x/rot.y. Wait/walk clips key `joint_21`, so the override is applied on `AnimationPlayer.mixer_applied` (after the mix). Body does not idle-turn toward the player. NPC eye height is per-species ROM data (not in the decomp source), so the port uses the head bone height.
