...
**Animation / lighting.** Rover's aisle move (`aNGD_move_to_aisle`) chase-turns the body toward the aisle point at 11.25°/frame while WALK1 carries him forward — no pivot-in-place. K.K. lighting matches `l_mEnv_kcolor_data_p_sel` (ambient 30/30/80, sun dir 0/89/79 colour 255/255/200, fog 100/100/120 kept disabled) with an authored ambient-energy lift for the white fur and a black clear instead of the unused `22/27/94`. **Train interior lighting** (`mEnv_SetBaseLight` `FIELD_DRAW_TYPE_TRAIN`): the outdoor `l_mEnv_normal_kcolor_data` palette for the current time (`Clock.outdoor_light()`), with **every** RGB field (ambient, sun, moon, bg) scaled by `sun_percent`, plus the `(35,30,40)×(1−sun_percent)` tunnel lift. So the tunnel is ambient (35,30,40) only on a black clear. Sun dir is fixed (0,90,80), moon (0,−30,−40) (`mEnv_ChangeDiffuseVctlSet`). The car point light (80,120,510) colour (255,255,160) power 1200 stays on all ride (no light-switch index). `ef_lamp_light` is a straight-down diffuse that chases (200,200,150) at +8/+8/+4 per frame while on and fades at −1/−1/−0.5 after `sunlight_flag`. The `Sun` / `Moon` / `LampLight` nodes live in `intro_train.tscn`; `IntroTrainPresentation` drives them. `sun_percent` ramp `1−√0.5 / 0.1 / 0.005`. **Rover motion**: speed ramps via `chase_f(speed, max, accel·0.5)` at 60 Hz, turns use `chase_angle` (talk 0x400, aisle/door 11.25°) while walking, and the body leans forward `speed·3640/3`. Clip morph −5 = 10 frames at 60 Hz (`standup_d1` blends; `open_d1` / `sitdown_d1` cut). `keitai_talk1` plays once, then `keitai_talk2` loops. Standup stays on the seat and `move_ready` snaps (100,300) afterwards. Return walk starts from the deck stop (140,130). **Camera** looks at `shadow_pos` (actor + `joint_0` root motion, base (0,1000,0)). Heights are floor-relative, since the decomp floor is y 40: eye/default look 40 above the floor, NORMAL look floor+30, TALK look shadow y+20 (`obj_look_y_max {30,20}`). The first-talk morph is `cKF_HermitCalc(r,1,0,1,3.2,0)` over 40 frames, with no re-morph at sitdown. Still open: Porter mouth-flap during the announce (no `mnk_1` face binding), K.K. blob-shadow tint (`shadow_color 0/30/70`).
...
- **Window scenery** (`ac_train_window` / `rom_train_out`, `scenes/ui/intro_train_car.gd`): every window combiner ignores SHADE, so the surfaces are unshaded and prim-tinted each frame with `clamp(global ambient + sun colour)` (`aTrainWindow_SetLightPrimColorDetail`). Sky = `TEXEL×PRIM`. Tunnel = `TEXEL×PRIM×43/255`. Cloud = `PRIM×127/255 + ENV(127,127,100)`, alpha `I4×xlu_alpha`. Tree = `TEXEL×(PRIM'×27/255 + ENV(60,60,35))`, with PRIM' offset (−80,−70,−160). Scroll offsets are `tex_scroll2` tile origins in 10.2 fixed point (raw/4 = texels), subtracted from the sample coordinate (negative UV offset). The tree scrolls `TreeScrollx` 500 +5/frame on a 128-wide tile. The cloud uses EVW `{0,−2}` = 0.25 texel/frame in T, from the start. On `sunlight_flag`, `DrawGoingOutTunnel` moves `Single_Scrollx` (tunnel, seg 11) and `Two_Scrollx1` (shineglass tile 0, seg 12) +15/frame to 1000. The sky and clouds are **not** exit-scrolled. **The tunnel reveals the scenery by clamping, not fading:** it is `TEX_EDGE` (alpha scissor), S-clamped, and its texture's first columns are transparent. Once the tile origin passes the texture width, every sample clamps onto the clear edge and the tunnel disappears. An opaque tunnel material or a repeating sampler keeps it covering the panes. Shineglass is the slanted light shaft (`train_shineglass.gdshader`): RGB PRIM, A = shine I4 (tile 0) × glass I4 (tile 1, (4u, v)) × `lod_factor`. Tile 1 uses the car's `rom_train_glass_tex` as a stand-in for the unextracted `_rgb_i4` bank. `xlu_alpha` add_calc (0.07) toward `aTW_GetNowAlpha`; `lod_factor` add_calc (0.3) toward 0–160. **Tree palette:** the bgtree CI4 LOADTLUTs `anime_3_txt` (seg 0x0A), which the actor binds to `aTrainWindow_tree_pal_table[aTrainWindow_GetTreePalletIdx()]` (15 date rows: bare, budding, cherry blossom, summer, autumn, bare). The pipeline used to fill that segment with the *field* tree TLUT (`_pick_dummy_palette` matched `tree`), which produced pale cream "snow" trees. `texbank.ACTOR_TLUT_TABLES` now binds the real table, and `_convert_actor_tlut_rows` writes `textures/rel/rom_train_bgtree_tex_pNN.png` for every row. These are native decodes: ACHD matched only some rows and ignored the TLUT. `intro_train_car.gd` picks the row from `Clock` month/day at load and again when `sunlight_flag` starts the exit, which is after the clock prompt, like `DrawInTunnel`.
...
Name and clock are small intro modals, not `m_ledit` / `m_timeIn` ports. K.K.'s sound/voice/rumble setup menus and staffroll frame sync are not ported; strum plays at a constant 0.5× and the guitar uses a bed + `intro_kk_arm` stem, muted on look-up (`Audio.set_ttkk_arm`). The returning-player / mask-cat Blanca path (`SCENE_PLAYERSELECT_2` / `SP_NPC_P_SEL2`) is not modelled.
...
- Title **Station Arrival** → `Game.start_intro_station()` → generated `world.tscn` + `IntroStationDirector` + `IntroStationStage`. The stage is shaped like `aID_*`: ride → get-off → Porter → one-unit walk → free walk → Nook call / approach / introduce / turn / lead → explain → pick → (house) → loan talk → Nook exit.
- **Coordinates are the decomp's.** The generated town keeps the fixed blocks (station (3, 1), player houses (3, 2) = `WorldGenerator.PLAYER_HOUSE_BLOCK`), so every landmark is the absolute decomp GX minus the station block origin (1920, 640), placed through `TownSpace`: player start (1970, 760), doorway / demo-camera centre (2180, 820), one-unit walk goal (2220, 840) @ 2.5, out-of-station z 970, Porter (2140, 820), Nook birth unit (8, 15) → (2260, 1260) facing (2320, 980), lead points (2240, 1300) → (2240, 1500), restart spots (block (3, 2) units 6/9 × 5/12, ±10 / +8), exit x 2240 split at z 1540, deleted past 1220 / 1980.
- **Train = the town train.** `aID_first_set` sets `train_coming_flag = 3`: `TrainService.request_arrival_demo()` → `TrainControl.demo_init` (x 2037, slow speed, `start_timer = now − 4:50`). `TrainService` runs during the arrival (`is_active` includes `intro_station_active`), so the world `FieldTrain` draws it with the sprung couplings (`TrainCars`), the engineer, the stop / door SEs and `KishaStatusTrg(0)`. The player is pinned to TRAIN1 + (60, 20, 20), yaw 0, every frame (`aTR1_passenger_ctrl`) until `WAIT_STOPPED`; the train then **leaves on its own ~20 s after the stop** (310 s dwell added to `now − 290`), usually during the Porter's line. The standalone debug acre steps its own `TrainControl` / `TrainCars` on its bound car nodes.
- **Get off** (`mPlayer_INDEX_DEMO_GETOFF_TRAIN`): `Player.begin_demo_getoff_train` plays OUTTRAIN1 with AnimationMove on XZ **and Y** (`cKF_ANIMATION_TRANS_Y`: `(joint_0.y − 1000)·scale` — a hop, net zero since the car floor is level with the platform); root motion carries the player 52 GX to z ≈ 812, and the state ends itself with the clip (`fromDemo_getoff_train` → demo wait).
- **Porter** (`station_porter.tscn`, `StationPorter.place_intro`, `mnk_1` face: blinks, mouth-flaps while he is the speaker): starts turned −90° (west), `GET_OFF_WAIT` turns him to the player at once, then `aSTM_look_player` (67.5° cone, TURN2 0x800). His force-talk `0x07DD` waits for the get-off to end and his current action to finish. After it: INTERRUPT_TURN east → INTERRUPT_MOVE (WALK one unit to (2180, 820), the platform gate) → THINK_4 (turn to player, look loop, normal talk `0x0831` / `msg_2097`).
- **One-unit walk** (`aID_walk_one_unit`): the player is demo-walked to (2220, 840) at 2.5 with the **demo camera still up**; only then does `Camera2_request_main_normal` hand over to `FollowCamera` and the stick come back (GO_OUT_OF_STATION).
- **Platform gate / forbid walls:** attributes 27…62 build unit-edge walls (`mCoBG_MakeForbidVectorData`, `FieldCollision.FORBID_WALLS`) for every actor. The platform row is `mCoBG_ATTRIBUTE_32` = a wall on its **track** (north) edge, and the stairs down beside the station carry 34 / 33 side rails. The gate unit (2180, 820) is opened with `STONE` at the get-off (`aID_ride_off_player_init`) and closed again when the Porter steps onto it (`aSTM_intro_demo_wait_init`); ordinary trains open it on `WAIT_STOPPED` and close it on `SIGNAL_STARTING` (`aTR1_chg_station_attr`). Runtime attribute changes go through `FieldCollision.set_attr_override` (`mCoBG_SetAttribute`).
- **Locomotive smoke / steam** (`aTR0_set_effect`, `aTR0_steam_work`, `FieldFx.Kind.KISHA_KEMURI` / `STEAM`): a smoke puff from the stack (+36, +110) every `12 / speed` frames (12 below speed 1), 80 frames, scale 0.003→0.027, prim (30,30,30) fading 200→4 after frame 40; piston steam on wheel frames 2–5 / 13–16 at (+15 / +22, +21, +42), angles 0xD000 / 0x0400, `ef_dust01` tiles by `Steam_tex_indx` / `Steam_plod_tbl`. None in the title demos. The smoke model + its two I4 tiles come from the pipeline (`ef_kisha_kemuri01`); effect frame formats are read from the `ef_*` model DLs (direct SETTIMG, or the one format a file's `anime_N_txt` loads use).
- **Nook** (`intro_rcn_guide.tscn`, moved by `NpcPointMove`): RUN = `aNPC_spd_data` max 3.0 / accel 0.3 (×0.5 per frame, `0.5·speed` GX per frame), turn 0x400; TURN / TALK_TURN spin 0x800 on WALK1; arrival = squared distance < 72; after an arrival he keeps moving on his speed (a re-issued point he is inside is simply run past — the exit relies on it). Every talk opens with `TALK_TURN` (the window waits until he faces the player); the demo `turn` flag only turns the player. CALL `0x07DE` NORMAL → APPROACH (run to player + 70 z) → INTRODUCE `0x07DF` TALK → TURN toward (2240, 1300) → TAKE_WITH (run (2240, 1300) → (2240, 1500)) with the player demo-walking after him at `rate × nook.speed` (1.0, or 0.6 inside 80 GX; min 0.5), crossing into the house acre with a **demo wade** (`Player_actor_CheckAbleMoveDemoWadeBlock`); when he stops (`action.idx == 0`) the player stops where they are → EXPLAIN `0x07E1` TALK.
- **House pick** (DECIDE_HOUSE_WAIT): `mPlib_Set_unable_wade(TRUE)` + `mCoBG_ChangeBlockBgCheckMode(1)` → `Player.unable_wade` / `intro_block_bg_check`: the house acre is walled 18 + 40 GX in and the wade border moves in a unit; pushing out raises `wade_cancelled` → STOP_WADE `0x07E2` (`msg_2018`, NORMAL). Normal talk to Nook here is `0x0820` (`msg_2080`, TALK). House-look `0x07E4` stays NORMAL, turn off.
- **Return** (`_1A4`): Nook is born at the claimed house's restart spot facing the player (`place_nook_for_restart`), waits for GO_OUT, then `0x07E6` (TALK). The bank chain parks on `msg_2078`'s `demo_order npc0[9]` ("…19,800 Bells!!!") via the runner's `advance_gate`: the window is suspended (`DialogueOverlay.set_suspended` — hidden and deaf to input, so A / Enter reach the pockets), the pockets open on the QUEST bag, the hand-over plays, then the window returns and the text runs on (`msg_2028`); closing the pockets empty jumps to the nag `0x07EB` (`msg_2027`), whose own order reopens them. Shop address slots: `{free1}` = row letter (`"QABCDEF"[bz]`), `{free2}` = column (1-based blocks).
- **Exit** (EXIT_TURN / EXIT): Nook spins toward (2240, 1900) or (2240, 1300), runs the leave points and is deleted; the music fades as he turns (`aID_retire_rcn_guide_wait_init`: `delete_ps_demo` + `fc_quiet`) and `intro_find_shop` starts once it is out, while he is still running. `Game.complete_intro_station` (loan, first job, unlock) runs after the delete.
- **Live check:** `$GODOT_BIN --path . res://scenes/dev/audit_station_arrival.tscn` (arrival → pick; `-- mode=resume` for the loan talk → payment → exit) logs each stage with player / Nook / Porter / car positions in decomp GX and writes `.tmp_captures/station_*.png`.
- Debug acre scene `scenes/ui/intro_station.tscn` remains for isolated checks (stub plots on the station lawn; Nook's lead points run off its single acre); play path is the generated world.
- **Music is cued by the text** (`BGMMAKE` / `BGMDELETE` control codes → `bgm_make` / `bgm_delete` dialogue events → `MessageBgm`, `mMsg_bgm_num` 1 ARRIVE / 2 RCN_GUIDE / 3 SELECT_HOUSE / 4 SELECT_HOUSE2 / 5, 7 Resetti, 0 / 6 quiet). The actors only start `INTRO_ARRIVE` (`aID_first_set_init`), `INTRO_RCN_GUIDE` on the house return (`_1A4`) and `INTRO_FIND_SHOP` as Nook leaves. Sequence: arrive theme until Nook's call `0x07DE` swaps to `INTRO_RCN_GUIDE` (walk to the plots, pick) → "It's decided then!" (`msg_2025`) `INTRO_SELECT_HOUSE` → "not nearly enough!!!" (`msg_2028`) quick quiet → the part-time offer `INTRO_SELECT_HOUSE2` → faded as Nook turns to go, then `INTRO_FIND_SHOP`. `aSTM_talk_wait` fades the arrive theme at z ≥ 970 only if the player talked to the Porter. `BGM_INTRO_SELECT_HOUSE2` is seq 65, an `AudioseqHeader` alias of seq 64 (zero size, `addr` = target); the audio pipeline resolves aliases (`resolve_seq_alias`).
