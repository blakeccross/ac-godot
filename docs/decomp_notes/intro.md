...
- Background sleep NPC at FG ut (4,4), birth offset x−6/z−24 → ~(174, 156) GX. Facing is **not** the `aNPC_think_in_block` appear-0 180°: `aNSO_set_request_act` requests `aNPC_ACT_CLAP` as `aNPC_ACT_TYPE_SEARCH` on `aNPC_ACT_OBJ_PLAYER`, so `aNPC_act_search_turn` turns the body to the seated player and keeps it there — on the GC frame the boar faces the camera and nods forward. `kokkuri_d1`/`d2` nod only (no `wait_nemu1`); bench align ~40 GX.
...
- Rover blinks and flaps his mouth by eye/mouth **texture swap**, not geometry (`aNPC_tex_anm_ctrl`): eyes run `eye_normal_blink` (open → shut 6f → half → open) in bursts of 1–3 with a 64–94 frame hold between, and the mouth only moves while `mMsg_Check_NowUtter` is true, i.e. while the window is still laying text in. Frames come from `--kind faces`.
