...
- Background sleep NPC at FG ut (4,4), birth offset x−6/z−24 → ~(174, 156) GX; `aNPC_think_in_block` appear 0 → decomp `shape_info.rotation.y` 180°; Godot host yaw **0** because `kab_1` `kokkuri_*` clips fold that turn into the baked rest (host PI + kokkuri reads backwards); `kokkuri_d1`/`d2` nod only (no `wait_nemu1`); bench align ~40 GX.
