...
9. **Rasterize** FG acres (bx 1–5, bz 1–6) to an 80×96 unit `WorldData`. When `grd_*.col.json` exists, water / sand / shoreline wave / slate / holes come from that table; otherwise geometric river strips and cliff bands. Wave attrs are sand, not water (`CheckWaveAttr`).
...
| --- | --- | --- | --- | --- |
| `HOUSE0`–`HOUSE3` (`ac_my_house.c`) | `(3,3)` `(12,3)` `(3,10)` `(12,10)` on `FG_TYPE_0069` / `GRD_S_F_MH_*` | west `+20` X, east `−20` X; both `+20` Z | 2×2 NW = FG (west) or FG+(−1,0) (east). Walk collision is a **4×4** `mCoBG` offset table (`aMHS_set_bgOffset`) with a **porch gap**; the door stand is `±48.29` GX (`aMHS_rewrite_pl_out_data`), not the 8 m AABB. Mesh yaw: west `WEST` (AC `angle_table` **+90°**), east 0. All four are `obj_s_myhome1` on new game (`mHm_HOMESIZE_SMALL`). |
