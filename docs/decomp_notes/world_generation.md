...
6. **Bridges / slopes** — `mRF_SetBridgeBlock`: one `GROUP_RIVER` acre **north** of the waterfall always becomes `*_BRIDGE` (`+7`, meshes `grd_s_r*_b_*`). 2-step towns also get a 50% **south** span. If that south span is missing, `mRF_SetSeaBlockWithBridgeRiver` turns the beach mouth into `BEACH_RIVER_BRIDGE` (`grd_s_m_r1_b_*`). Then one cliff acre per river side becomes a slope. The Tortimer extra actor (`BRIDGE_A0` / `obj_s_bridgeA`) is a later event, not new-game landform.
