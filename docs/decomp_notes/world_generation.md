...
`WorldGrid` remains the runtime occupancy + walkability board. The builder copies terrain into it, then `place()`s buildings and objects. `FieldCollision` is a height query plus walls: catalog corners for Y (including river units), infinitely thin XZ segments on terrace/bank edges (plus 45° slate diagonals), and `revise_xz` as circle-vs-segment so the player slides along water and cliffs instead of catching physics shapes.
...
5. **Beach** F-row
6. **Bridges / slopes** — `mRF_SetBridgeBlock`: one `GROUP_RIVER` acre **north** of the waterfall always becomes `*_BRIDGE` (`+7`). 2-step towns also get a 50% **south** span. If that south span is missing, `mRF_SetSeaBlockWithBridgeRiver` turns the beach mouth into `BEACH_RIVER_BRIDGE` (`grd_s_m_r1_b_*`). Then `mRF_SetSlopeBlock` turns **one cliff per river side on every cliff band** (each west `LEFT_TRANSITION`) into a slope — two-step towns need a ramp on the lower terrace or the beach is unreachable. Wood vs stone is **not** a second actor: `mRF_SelectBlock` picks a `data_combi` BG of that type (`grd_s_r*_b_*`). `_b_1` (and west `_b_3` / beach `_b_3`) use `bridge_1_tex` + collision attrs 32–35 (stone); `_b_2` and most `_b_3` use `bridge_2_tex` + attrs 27–31 (wood). Unused combis are preferred so two spans of the same type do not clone one mesh. The Tortimer extra actor (`BRIDGE_A0` / `obj_s_bridgeA`) is a later event, not new-game landform.
7. **Uniques** — wishing well / police / museum on flats below cliff; shop+post on A-row dumps. Able Sisters + dock on the beach row.
8. **Perfect-bit retry** — `mRF_MakePerfectBit` / the `while` in `mRF_MakeRandomField_ovl`: if any of slope L/R, bridge upper/lower, shrine, police, museum, pool, or Able Sisters failed to place, discard the landform and regenerate (same 2/3-step mode, RNG continues). Without this, some seeds ship with no museum.
9. **Heights** — bump when crossing a terrace step going north (`CLIFF_HORIZONTAL` / `TOP_RIGHT` / `TOP_LEFT` bits, same as `mRF_GetBlockBase`). Vertical and bottom-corner river-cliffs stay on the current terrace.
10. **Rasterize** FG acres (bx 1–5, bz 1–6) to an 80×96 unit `WorldData`. When `grd_*.col.json` exists, water / sand / shoreline wave / slate / holes come from that table; otherwise geometric river strips and cliff bands. Wave attrs are sand, not water (`CheckWaveAttr`).
11. **Acre meshes** — `FieldCatalog.acre_for_block_type` picks a `grd_*` from the matching `data_combi` family (flat `grd_s_f_*`, river `grd_s_r1_*`, house `grd_s_f_mh_*`, bridge `grd_s_r*_b_*`, …), preferring a BG this town has not used yet (`mRF_SelectBlock` / `l_use_data`). Names are assigned even when the GLB is missing so fallback decks can still be wood vs stone. Variants whose sidecar is a HEIGHT_MAX filler (dummy TRACKS rows that reused the mesh) are skipped so they cannot box the acre in walls.
...
BG and FG are picked separately — BG by type family, then an FG that matches that BG — rather than one bit-exact `data_combi_table` row lookup for both together; there is no acre streaming (the whole town loads). The FG catalog stays gitignored like other disc output.
...
| --- | --- | --- | --- | --- |
| `HOUSE0`–`HOUSE3` (`ac_my_house.c`) | `(3,3)` `(12,3)` `(3,10)` `(12,10)` on `FG_TYPE_0069` / `GRD_S_F_MH_*` | west `+20` X, east `−20` X; both `+20` Z | 2×2 NW = FG (west) or FG+(−1,0) (east). Walk collision is a **4×4** `mCoBG` offset table (`aMHS_set_bgOffset`) with a **porch gap**; interact check `−20,+20` local GX, exit `±48.29` GX (`aMHS_rewrite_pl_out_data`), not the 8 m AABB. Mesh yaw: west `WEST` (AC `angle_table` **+90°**), east 0. All four are `obj_s_myhome1` on new game (`mHm_HOMESIZE_SMALL`). |
...
| Villager house | SIGN reserve (`mNpc_SetNpcHome`); SIGN ut 1..14; **6** homes (`mNpc_LOOKS_NUM`) | none (`ac_house`) | 3×3 RSV around SIGN overwrites FG trees. Mesh `obj_s_house{1-5}_{a-e}` from resident `house_type` / `house_palette`. New game also places one starter villager in the yard of each house (`mNpc_DecideLivingNpcMax`) |

The house acre always has HOUSE0–3 (top-left, top-right, bottom-left, bottom-right). New game inits all four to tent size; only player 0’s private data is filled. Mailboxes (`ACTOR_PROP_MAILBOX0`–`3`) sit two units toward the acre center on the house row; haniwa two units south of each house. All four mailboxes are spawned by `WorldGenerator`/`scenes/world/mailbox.tscn`; only the one in front of the plot the player owns is live. Ownership is `mHS_get_arrange_idx` — the plot picked in the station intro, not always HOUSE0 — and lives in the saved player `House.outdoor_building_id` (`PlayerHouse.owned_building_id`, written by `Game.claim_intro_house`); mailbox, gyroid, house model and map label all read it. Haniwa (`ACTOR_PROP_HANIWA0`–`3`, FG ut (3,5) / (12,5) / (3,12) / (12,12)) are spawned by `WorldGenerator._place_house_haniwa` as `scenes/world/haniwa.tscn` — see [world objects](world_objects.md#house-gyroids-ac_haniwa). Door of the west pair uses AC **+90°** Y (`WorldGrid.Facing.WEST` here — `EAST` is −90°). `mFI_PullTanukiPathTrees` then clears trees on **C-3** (`Save_Get(fg[2][2])`), not B-3: ut indices `0x07, 0x08, 0x17, 0x18, 0x27, 0x28`.
...
Generated towns instance one `grd_*` GLB per acre under `Terrain/Acres` for the full 7×10 block grid: FG (5×6), left/right border cliffs (`grd_s_e2_*` / `e3_*`), north border (`e1` / corners `e4`/`e5`), and south ocean / island rows (`grd_s_o_*`, `il`/`ir`; `SEA_EXCEPTIONAL` mirrors the beach acre above as its `m`→`o` pair). Placement matches `ac_field_draw`: min-corner (`mFI_BkNum2WposXZ`), Y = `mFI_BkNum2BaseHeight` (height × 3 cells = 6 m). Draw-list verts are 16×; the original undoes that with `Matrix_scale(0.0625)` while actors use `Matrix_scale(0.01)`. The pipeline writes both at `0.001`; `FieldCatalog` applies the two draw scales into the same 40 GX = 2 m grid so acres, trees, and the player share one GX factor. Each acre job now includes OPA `*_model` and XLU `*_modelT` (river/ocean water). `GeneratedVisual` keeps grass clamp-baked and swaps scrolling shaders on water/wave/wet-sand surfaces. The player does **not** stand on those triangles. Each `grd_*` combo ships gfx **and** a 16×16 `mCoBG` table (`data_bgd`); the pipeline writes that as `grd_*.col.json` next to the GLB. `FieldCollision` bilinear-samples those corners (`GetBgY_AngleS_FromWpos`) — **including water** — and treats terrace/bank edges as **infinitely thin XZ segments** (`mCoBG_SearchWallFlag`), plus 45° walls on slate units (`GetUnitVecInf_SlatingWall`). Mixed slate/grass cardinals are shortened to the half-edge that had a real height jump before `UtInf2NormalSlateWallVector` flatten, so a full N–S wall does not sit in the SE notch in front of the 45° face. `revise_xz` resolves a circle against those segments so actors slide instead of catching 3D box or triangle corners. Trees, buildings, and map bounds stay physics. Authored ponds and geometric cliff faces (no sidecar) stay holes. Snapping to the unit *center* buried the capsule in the ramp mesh and blocked downhill travel. Off-map is impassable (`MapBound`). Test town places a single `grd_s_f_1` the same way and keeps its authored pond. Colored paint tiles are only used when those meshes are missing.
...
| Villager house | `obj_s_house{1-5}_{a-e}` (`ac_house`) | `obj_s_houseN_X.glb` (falls back to `obj_s_houseN.glb`) |
