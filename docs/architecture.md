...
| `Fishing` | Cast → float → nibble → bite → hook; owns the one live session and its bobber |
| `FishCatalog` | `data/creatures/*.tres` → what can bite at this month and hour, weighted |
| `FishSize` | The `ac_gyo_test.c` per-size and per-species tables, in metres and seconds |
| `FishShadow` | One shadow's action machine: wait, swim, near, touch, bite, comeback, escape |
| `FishSchool` | The field's two live shadows and their escape puffs; spawns and culls them |
