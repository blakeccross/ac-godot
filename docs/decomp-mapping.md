...
| `m_npc`, `m_npc_schedule`, `m_npc_walk` | Looks-based daily tables; one NPC actor; new town picks `mNpc_LOOKS_NUM` starters (one personality each) from a shuffled starter pool | `VillagerData` + `VillagerPersonality` + `ScheduleData` + `Villager` scene; `VillagerSchedule` / `VillagerAI` (reusable actions) / `VillagerCatalog.pick_starters` |
