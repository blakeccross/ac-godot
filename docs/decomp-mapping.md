...
| `ac_field_draw` / `evw_anime` | Acre OPA land + XLU water; river dual UV scroll; ocean 300-frame wave cosine; wet-sand env | River: `GeneratedVisual` `river_water` on `grd_*_modelT`. Wet sand: `beach_wet.gdshader` env pulse. Ocean XLU: imported GLB (stripped on marine acres). `--kind water` |
