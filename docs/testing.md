...
- **Do:** Resource defaults, clock advancement, 06:00 renew, shop hours, catchable windows, inventory stacking, shop prices, schedule lookups, villager looks tables / talk friendship / greeting pick, activity plans (wake / leave / walk-to-acre / looping wander / go home / sleep), starter roster pick (one per looks), field roam goal tables, standable cells, wait/walk/run weights, shared 1.5 / 4.5 m/s walk/run (`VillagerMotor`), and wander arrive (`VillagerWalk`), save round-trips, grid cell math, locomotion accel/turn, interaction verb priority and host duck-typing, equipped-tool field verbs (`ToolUse`), held-tool `visual_id` / clips, tree chop/shake/stump (`TreeUse`), plant growth stages / water / fruit (`PlantGrowth`), shovel dig/fill holes (`HoleUse`), generator determinism (`seed` → same `WorldData.fingerprint()`), unit heightfield (`FieldCollision`) and catalog water units when `grd_*.col.json` is present, world-object registry kinds and decomp acre placement (house B-3, shop row A, museum unique, Able Sisters beach).
- **Don't:** Screenshot-test placeholder art, or require the boot scene for logic tests.

Suites live in `tests/unit/`: `test_clock.gd`, `test_inventory.gd`, `test_save_service.gd`, `test_game.gd`, `test_world_grid.gd`, `test_world_generation.gd`, `test_field_catalog.gd`, `test_field_collision.gd`, `test_player_locomotion.gd`, `test_interaction.gd`, `test_tool_use.gd`, `test_tree_use.gd`, `test_plant_growth.gd`, `test_hole_use.gd`, `test_resources.gd`, `test_villager.gd`.
