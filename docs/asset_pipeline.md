...
| Grass colour / tree snow ignore season (U key) | GLBs bake one season into albedo. Build the seasons pack (`--kind seasons`) then press **U**. Runtime swaps grass/earth/cliff/bush/leaf/trunk albedos from `environment/seasons/{s,f,w}/`. Cliff fringe (`bush_a`/`bush_b`) comes from the field BG bank, not flat acres — rebuild seasons after pipeline changes. Autumn recolors summer CI; winter needs the winter field bank + `obj_w_tree*` leaf art in that pack. Mesh remap alone is not enough when only summer GLBs exist. |
