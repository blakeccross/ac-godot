...
func _run() -> void:
	...
	player.call("set_facing", WorldGrid.yaw_for_facing(WorldGrid.Facing.NORTH))
	await _frames(4)
	print("start   chair ", chair_entry.cell)
	await _shot("0_start")

	print("grip    ", player.call("_try_grip"))
	...
	await _frames(4)
	print("released gripping=", player.get("_gripping"), " busy=", player.get("_busy"))
	...
	Input.action_press("move_forward")
	await _frames(70)
	print("sit     rest=", (player.get("_rest") as Dictionary).get("phase"), " pos cell ", session.grid.world_to_cell(player.global_position))
	...
	Input.action_release("move_back")
	await _frames(30)
	print("stand   rest empty=", (player.get("_rest") as Dictionary).is_empty(), " busy=", player.get("_busy"), " cell ", session.grid.world_to_cell(player.global_position))
	await _shot("7_stood")
	...
	var probe_hit: InteractionQuery = player.call("_resolve_interact")
	...
	print("tap     grip=", player.call("_try_grip"))
	Input.action_press("interact")
	await _frames(5)
	Input.action_release("interact")
	await _frames(40)
	var ui: Node = get_tree().get_first_node_in_group("dialogue_ui")
	print("chest   dialogue open=", ui.call("is_open"))
	await _shot("8_chest_talk")
	ui.set("_choice_index", 1)
	for _i: int in 12:
		ui.call("fast_advance")
		await _frames(20)
	await _frames(60)
	print("chest   stored=", chest_entry.stored.size(), " apples ", apples_before, "->", Game.inventory.count_of(&"apple"), " busy=", player.get("_busy"))
	...
	player.call("set_facing", atan2(-1.0, 0.0))
	await _frames(4)
	print("east    grip=", player.call("_try_grip"))
	Input.action_press("interact")
	...
	await _frames(30)
	print("goki    out=", get_tree().get_nodes_in_group("house_goki").size(), " stored=", Game.interiors.player_house().goki_count, " startled busy=", player.get("_busy"))
